...
with st.sidebar:
    st.sidebar.subheader("Standaard weergave")
...
with col3:
    if show_hrv:
        ...
        # Bereken RMSSD per rustblok
        rmssd_df = bereken_rmssd_rust(df_ecg_pieken, rust_intervallen)

        # Optie: verloop tonen
        toon_verloop = st.checkbox("Toon RMSSD-verloop over tijd", value=False, key="hrv_verloop")

        fig = plot_rmssd_rust(rmssd_df, toon_tijdverloop=toon_verloop)
...
with col4:
    if show_sdnn:
        ...
        # Add checkbox for time series view
        toon_verloop = st.checkbox("Toon SDNN-verloop over tijd" , value=False, key="sdnn_verloop")
        
        # Create visualization
        fig3 = visualize_sdnn_per_block(df_ecg_pieken.copy(), toon_verloop)
